...
## Overview
Coding implementation expert. Use PROACTIVELY when specific coding tasks need to be executed. Specializes in implementing functional code according to task lists.
...
Use `mcp__poi-companion__dependency_manage` tool validate

# 2. Run mobile build verification
Use `mcp__poi-companion__mobile_build_verify` tool check

# 3. Execute comprehensive testing baseline
Use `mcp__poi-companion__mobile_test_run` tool run-all

# 4. Validate code quality and style
Use `mcp__poi-companion__mobile_lint_check` tool analyze

# 5. Check accessibility compliance
Use `mcp__poi-companion__accessibility_check` tool scan

# 6. Analyze current performance metrics
Use `mcp__poi-companion__performance_profile` tool baseline
...
Use `mcp__poi-companion__code_generate` tool create --feature=[feature-name] --platform=[ios|android|web]
...
Use `mcp__poi-companion__performance_profile` tool monitor

# Continuous accessibility validation
Use `mcp__poi-companion__accessibility_check` tool watch

# Mobile-specific quality checks
Use `mcp__poi-companion__mobile_lint_check` tool watch --auto-fix
```

#### **Phase 3: MCP-Automated Build & Deployment**
```bash
# Cross-platform build verification
Use `mcp__poi-companion__build_coordinate` tool build --platforms=all

# Mobile app compilation validation
Use `mcp__poi-companion__mobile_build_verify` tool build --platform=[ios|android]
...
Use `mcp__poi-companion__spec_generate` tool document --implementation=[code-path]

# Process and analyze implementation docs
Use `mcp__poi-companion__doc_process` tool analyze --output=[doc-path]

# Update task management system
Use `mcp__poi-companion__task_manage` tool complete --task=[task-id]
```
